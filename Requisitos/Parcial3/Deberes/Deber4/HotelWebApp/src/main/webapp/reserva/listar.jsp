...
<head>
    ...
    <title>Reservas - HotelGestión</title>
    ...
    <div class="content-wrapper">
        <div class="container">
            <div class="page-header d-flex justify-content-between align-items-center">
                <div>
                    <h1><i class="fas fa-calendar-check me-2" style="color: var(--gold);"></i>Reservas</h1>
                    <p class="subtitle">Gestión de reservas y estadías</p>
                </div>
            ...
            <div class="table-container">
                <div class="search-box mb-3">
                    ...
                    <input type="text" class="form-control" id="tableSearch" placeholder="Buscar reservas..." onkeyup="filterTable()">
                </div>
                <table class="table" id="dataTable">
                    ...
                    <tbody>
                        <c:forEach var="r" items="${reservas}">
                            <tr>
                                <td><span class="fw-semibold">${r.idReserva}</span></td>
                                <td><i class="fas fa-user me-1 text-muted"></i>
                                    <c:forEach var="c" items="${clientes}">
                                        <c:if test="${c.idCliente == r.idCliente}">${c.nombre} ${c.apellido}</c:if>
                                    </c:forEach>
                                </td>
                                <td><i class="fas fa-door-open me-1 text-muted"></i>
                                    <c:forEach var="h" items="${habitaciones}">
                                        <c:if test="${h.idHabitacion == r.idHabitacion}">${h.numero}</c:if>
                                    </c:forEach>
                                </td>
                                <td><i class="fas fa-calendar-plus me-1 text-muted"></i>${r.fechaEntrada}</td>
                                <td><i class="fas fa-calendar-minus me-1 text-muted"></i>${r.fechaSalida}</td>
                                <td>
                                    <span class="badge ${r.estado == 'Confirmada' ? 'bg-success' : r.estado == 'Cancelada' ? 'bg-danger' : 'bg-secondary'}">
                                        <i class="fas ${r.estado == 'Confirmada' ? 'fa-check-circle' : r.estado == 'Cancelada' ? 'fa-times-circle' : 'fa-check-double'} me-1"></i>
                                        ${r.estado}
                                    </span>
                                </td>
                                <td><span class="fw-semibold" style="color: var(--gold);">$${r.total}</span></td>
                                <td class="text-center">
                                    <a href="?action=editar&id=${r.idReserva}" class="btn btn-sm btn-warning me-1" title="Editar">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="javascript:void(0)" onclick="confirmDelete(${r.idReserva}, 'Reserva #${r.idReserva}')" class="btn btn-sm btn-danger" title="Eliminar">
                ...
                <c:if test="${empty reservas}">
                    <div class="text-center py-4 text-muted">
                        <i class="fas fa-calendar-check fa-3x mb-3" style="color: var(--gold);"></i>
